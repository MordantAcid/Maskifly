...
- **Безопасный режим аудита (NEW)** – в лог попадает только временная метка и хеш (SHA256) исходного значения
- **Асинхронный неблокирующий аудит (NEW)** с очередью и фоновым потоком
...
- **`Tensor`** – многомерный массив (обёртка над `numpy`) с поддержностью autograd
- **Автоматическое дифференцирование** – градиенты скалярных функций через `.backward()`
- **Базовые операции**: сложение, умножение, матричное умножение, возведение в степень, ReLU, экспонента **(NEW `.exp()`)** , логарифм **(NEW `.log()`)** , изменение формы, суммирование по оси, **среднее (`.mean()`) (NEW)** , **объединение (`.stack()`) (NEW)**
...
- **Функция `is_grad_enabled()` (NEW)** – проверка состояния вычисления градиентов

### Интеграция с веб-фреймворками

- **Django**: middleware для маскировки `GET`/`POST` и JSON тела запроса
- **FastAPI**: middleware и декоратор для маскировки JSON-ответов, **с ограничением размера ответа (NEW: `max_size_bytes`, `skip_on_too_large`)**
...
audit = AuditLogger(
    ...
    drop_on_full=False,   # NEW: если True – терять события при переполнении
    queue_timeout=0.1     # NEW: таймаут блокирующей вставки
)
...
FastAPI
Добавьте middleware для маскировки всех JSON-ответов с возможностью ограничения размера:
...
setup_fastapi_masking(
    ...
    max_size_bytes=10_485_760,   # 10 MB, NEW
    skip_on_too_large=True       # при превышении – пропустить маскировку, NEW
)
...
Также доступна middleware-фабрика MaskResponseMiddleware с параметрами max_size_bytes и skip_on_too_large.
...
# Дополнительные операции (NEW: exp, log, mean, stack)
x = Tensor([1.0, 2.0, 3.0], requires_grad=True)
y = (x ** 2).relu().exp().log()
y.mean().backward()      # среднее значение и обратное распространение
print(x.grad)

# Объединение тензоров (NEW)
